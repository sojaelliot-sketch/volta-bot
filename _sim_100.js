...
async function run(i) {
  const jid = `sim_${i}`;
  const sock = { sendMessage: async()=>{}, groupMetadata: async()=>({subject:'T'}) };
  try {
    const s = await ms.startMatch({
      sock, msg: {key:{remoteJid:jid}, message:{conversation:'!play'}},
      jid, sender: OWNER, cmd: 'play', args: [], user: User.getByWhatsappId(OWNER),
    });
    if (!s) { fail++; return; }
