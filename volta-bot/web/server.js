...
const { PACKS, SHOP, TRAINING, PENALTY, COINFLIP, HIGHLOW, SLOT, RARITY, MARKET, ACADEMY } = require('../config/constants');
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/info' && req.method === 'GET') {
      return send(res, 200, { ok: true, version: WEB_VERSION, features: ['shop', 'penalty', 'coinflip', 'slot', 'highlow', 'market', 'transfer', 'search', 'academy', 'tournament', 'heartbeat'] });
    }
