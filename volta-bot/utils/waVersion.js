 ...
 *
 * fetchLatestBaileysVersion() can return an OUT OF DATE version while reporting
 * isLatest: true. A stale version lets the socket connect, issue a pairing code,
 * then silently fail at the final linking step with:
 *     "Couldn't link device — An error occurred. Try again."
 *
 * Resolution order (first one that works wins):
 *   1. WA_VERSION env override
 *   2. fetchLatestWaWebVersion()  — the correct source of truth
 *   3. fetchLatestBaileysVersion() — only if not a known-stale build
 *   4. KNOWN_GOOD — pinned fallback
 */

const KNOWN_STALE = [2, 3000, 1035194821];
...
async function resolveVersion(baileys, log = () => {}) {
  ...
  // 2. live version WhatsApp Web is actually serving
  ...
  // 3. Baileys helper, skip if known-stale
  if (typeof baileys.fetchLatestBaileysVersion === 'function') {
    try {
      ...
      if (same(v, KNOWN_STALE)) {
        log(`Ignoring stale version v${v.join('.')} — WhatsApp refuses new device links on it.`);
      }
    } catch (err) {
